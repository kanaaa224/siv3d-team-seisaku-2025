...
void End::update()
{
	...
	if ((m_startButton.leftClicked() && m_selectedButtonIndex == 0) || (controller.buttonA.down() && m_selectedButtonIndex == 0))
	{
		...
		changeScene(SceneState::Result, 0.5s);
	}
	//// マウス左クリックまたはコントローラーのAボタンで決定
...
void End::draw() const
{
	...
	// end描画
	FontAsset(U"TitleFont")(U"GameClear").drawAt(TextStyle::OutlineShadow(0.2, ColorF{ 0.1, 0.1, 0.1 }, Vec2{ 3, 3 }, ColorF{ 0.0, 0.5 }), 100, Vec2{ 640, 150 });
	...
	// ボタン描画
	{
		...
		boldFont(U"RESULT").drawAt(25, m_startButton.center(), ColorF{ 0.1 });
